...
# A button
        ...
        if type & 0x02:
            ...
            if axis:
                ...
                button = button_map[number]
		if axis==K_BASED:
		   pin12.write(90-(fvalue*0.1)*90)
                   print "pin12 : %s %s"%(fvalue,90-(fvalue*0.1)*90)
		if axis==K_END:
		   pin10.write(90-(fvalue)*90)
                   print "pin10 : %s %s"%(fvalue,90-(fvalue)*90)
		if axis==K_UP2:
		 current_rz=current_rz-fvalue
	          ...
	          current_rz=0
		 pin13.write(current_rz)
              	 print "pin13 %s: %.3f" % (axis, current_rz)
		if axis==K_UP1:
		 ...
		 if (current_z <0):
		  current_z=0
		 pin11.write(current_z)
                 print "pin11: %s %.3f"%(pin16.read(),current_z)
